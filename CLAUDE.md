...
- **App-store wrapper:** Capacitor. Scaffolded in `ios/` + `android/` (bundle `com.millrace.globalrecall`). `www/` is a gitignored copy generated by `npm run cap:copy` — `index.html`/`sw.js`/`manifest.json` stay the single source of truth. Run `npm run cap:sync` after editing them to refresh both native projects. Submission to TestFlight / Play internal track is still pending (needs Apple Developer Program enrollment + Google Play Console account).
...
- Capacitor wrap scaffolded (see Stack above). Remaining: Apple Developer Program enrollment, Google Play Console account, first TestFlight + Play internal-track submissions — all gated on the owner's own store accounts/credentials.
